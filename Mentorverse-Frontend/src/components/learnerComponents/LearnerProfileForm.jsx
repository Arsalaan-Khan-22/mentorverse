    ...
    <form className="bg-white p-12 rounded-2xl shadow-(--shadow-primary) flex flex-col gap-7">
      <div className="flex flex-col gap-3">
        ...
        <input
          ...
          className={
            !errors.name
              ? "w-full border border-[rgba(0,0,0,0.15)] px-5 py-3 rounded-lg focus:shadow-(--shadow-primary) focus:-translate-y-px transition-all duration-100 focus:outline-(--primary-color) " +
                (isDisabled &&
                  "cursor-not-allowed bg-(--bg-color) text-gray-600 ")
      ...
      <div className="flex flex-col gap-3">
        ...
        <textarea
          {...register("bio", {
            // required: true,
            // validate: (value) =>
            //   value.trim() !== "" || "Bio can not be empty or spaces only",
          })}
          className={
             "w-full border border-[rgba(0,0,0,0.15)] px-5 py-3 rounded-lg focus:shadow-(--shadow-primary) focus:-translate-y-px transition-all duration-100 focus:outline-(--primary-color) " +
                (isDisabled &&
      ...
      <div className="flex flex-col gap-3">
        <label htmlFor="interests">Interests (comma-seperated)</label>
        <input
          {...register("interests", {
            required: true,
            validate: (value) =>
