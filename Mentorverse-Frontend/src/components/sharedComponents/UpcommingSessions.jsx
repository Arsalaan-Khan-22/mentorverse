...
const UpcomingSessions = ({ upcomingSessions, onCancelSession, mode }) => {
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10 rounded-2xl bg-white w-full lg:w-1/2 shadow-(--card-shadow) outline-(--primary-color) hover:outline-2 hover:outline-(--primary-color) hover:shadow-2xl transition-all duration-100">
      <div className="flex flex-col sm:flex-row justify-between gap-4 sm:gap-10 mb-6 sm:mb-12">
        ...
        <Link
          to={mode === "mentor" ? "/mentor/bookings" : "/learner/bookings"}
      ...
      {upcomingSessions.length === 0 ? (
        ...
        upcomingSessions.map((session, index) => {
          ...
          return (
            <div
              key={index}
              className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-(--bg-color) p-4 sm:p-5 rounded-2xl mb-4"
            >
              <div className="flex items-center gap-4 sm:gap-5">
                <img
                  className="w-12 sm:w-16 border-2 border-(--primary-color) rounded-full"
                  src="https://api.dicebear.com/7.x/avataaars/svg?seed=Sarah"
                  alt="dummy avatar"
                />
                <div>
                  ...
                  <p className="text-gray-700 text-xs sm:text-sm">
                    {sessionMonth} {sessionDate}, {sessionYear} at {hours}:{minutes} {ampm}
                  </p>
                </div>
              </div>
              {mode === "learner" && (
                <button
                  onClick={() => onCancelSession(session.bookingId)}
                  className="mt-3 sm:mt-0 border-2 rounded-md text-xs sm:text-sm font-semibold text-red-500 px-4 sm:px-7 py-2 sm:py-3 cursor-pointer"
                >
                  Cancel
                </button>
              )}
