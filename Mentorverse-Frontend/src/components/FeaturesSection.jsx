...
const FeaturesSection = () => {
  ...
  return (
    <div className="bg-(--bg-color) py-20 text-center">
      <h2 className="features-heading text-3xl sm:text-4xl lg:text-5xl font-extrabold relative w-fit justify-self-center pb-4">
        Why Choose Mentorverse?
      </h2>

      <div className="pt-12 px-6 sm:px-12 lg:px-24 grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-8">
        {cardContent.map((card, index) => (
          <FeatureCard
            key={index}
            icon={card.icon}
